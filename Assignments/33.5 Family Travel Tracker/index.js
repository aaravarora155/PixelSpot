...
async function dbInit() {
  ...
  if (parseInt(userCountResult.rows[0].count) === 0) {
    // We won't seed defaults anymore as they need an email, or we seed with a null/placeholder email
...
app.get("/", async (req, res) => {
  ...
  if (users.length === 0) {
    // If no users exist for this email, we might want to redirect to /new or show a limited view
    return res.render("new.ejs");
  }

  // Ensure currentUserId is valid for this user
...
app.post("/new", async (req, res) => {
  const email = req.user ? req.user.username : null;
  try {
    const result = await db.query("INSERT INTO users (name, color, email) VALUES ($1, $2, $3) RETURNING id", [req.body.name, req.body.color, email]);
